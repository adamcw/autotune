...
import shutil
import itertools
from processruns.runner import permute_runs, run_command
...
if not args['--input']:
    ...
    if not os.path.isfile(args['--input']):
        sys.stderr.write("Could not find input file: %s" % args['--input'])
        sys.exit()
...
for r in runs:
    if not os.path.isfile(r['executable']):
        sys.stderr.write("Could not find executable: %s" % r['executable'])
        sys.exit()

    if not os.path.isdir(r['ems']):
        sys.stderr.write("Could not find error model directory: %s" % r['ems'])
        sys.exit()
    ...
    if i > 1:
        ...
        if not os.path.isfile(filename):
            if not args['--quiet']:
                sys.stderr.write("Could not find output of previous run: %s" % filename)
            continue
        ...
        if h_boot:
            if len(headers) != 2 or 'new t_check' not in headers[-1]['opts']:
                if not args['--quiet']:
                    sys.stderr.write("Error: Original boot process has not completed. %s" % filename)
                continue
            ...
            if 't_check' not in headers[0]['opts']:
                if not args['--quiet']:
                    sys.stderr.write("Error: Unable to parse t_check from original run: %s" % filename)
                continue
